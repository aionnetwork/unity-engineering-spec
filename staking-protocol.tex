...
\begin{tikzpicture}[->,>=stealth',auto,node distance=5cm,transform shape]
  ...
  \node[elliptic state, align=center] (B) [right of=A] {$delegated$};
  ...
  \node[elliptic state, align=center, node distance=1.5cm] (C) [below of=B] {$bonded$};
  
  \path (A) edge [align=center,above] node {delegate} (B)
        (B) edge [align=center,above] node {undelegate} (D)
        (A) edge [align=center,below] node {bond} (C)
        (C) edge [align=center,below] node {unbond} (D)
        ...
        (D) edge [align=right,bend left=60, pos=0.25]  node {$>$ unstake lockout} (A);
\end{tikzpicture}
\end{center}
...
\subsection{Delegator Actions in Pool Registry}
...
\begin{itemize}
    ...
    \begin{itemize}[label=--,nosep]
        \item Send a \say{message} to their delegators by updating their pool meta-data, that they will no longer wish to be an active pool. 
...
\begin{itemize}
    \item \textit{Self-bond minimum}: This refers to the minimum amount of bond required to be \say{put up} by a staker in the Staker Registry. See \S\ref{self_bond} for details.
...
The delegators are modelled as the most unsophisticated users of the system. For these users, two-step transactions system represent a source of severe friction. In order to keep the user experience as \say{familiar} (w.r.t traditional web services) as possible, incentive mechanisms are designed to shift the responsibility of calling the finalization transaction away from the delegator initiating the asynchronous transactions. 

The insight enabling this mechanism stems from the idea that, once an \say{asynchronous} transaction has been initiated by an authorized user, finalization can technically be performed by anyone in the system (since it is not an authorized action).
...
\paragraph{Staker registry symmetric-key rights} \label{km_staker} A  (private) staker required to manage the following four (4) keys, each of which correspond to a distinct function: 
\begin{itemize}
...
Note that the pool operator is shielded (by the Pool Registry contract) from much of the complexity of managing the extra keys associated with the operation of a private staker through the Staker Registry. 
...
The repercussion for leaving a pool in the \textit{broken} state (imposed on the pool operator) is that the pool commission is set to zero (ie. the pool earns no commission). Note: The commission set by the operator is still \say{remembered}, but only realized as 0 until the invariant is restored.

\subsection{Protocol (Kernel) Responsibilities}
Since the function of the protocol depends on the Staker Registry contract's state, there must be some special interactions that must to coded into protocol implementation (i.e. \say{the kernel}). 
