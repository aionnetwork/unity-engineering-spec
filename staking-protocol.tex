...
At any point, a delegator or a voter can choose to withdraw their bonded coins (reducing the PoS power of the staker delegated or voted to). When a delegator withdraws their delegated coins, they are said to \textit{undelegate}. When a voter withdraw their voted coins, they are said to \textit{unvote}. In either scenario, when the coins in question get unbonded in the staking system, they are subject to a post-lock period (\cite{WZS19} \S4.1.1). After the post-lock period has elapsed, their coins exit the staking system and become \textit{liquid} (free to transfer to other users). 

A delegator is allowed to \textit{transfer} their delegation between stakers (subject to stipulations discussed in a subsequent section). 
...
\begin{itemize}
    ...
    \end{itemize}
    This contract implements the Staker Registry Listener, which is important for the StakerRegistry contract to notify the PoolRegistry of changes about changes to the signing address and coinbase address. 
    ...
    \item \textbf{\textit{Staker Registry Listener}}: This interface is implements two basic listeners (on
\end{itemize}
...
\end{itemize}
Since we don't have an onBlockProduced function, some of these actions have to get separated across transactions. (ie. all these things require timeouts) .

\subsubsection{Pool States: Broken and Active}
...
The major requirements around rewards sharing was that \say{rewards sharing must be an on-chain, automatic process that does not require any action on part of the stake pools}. 
...
Copy over the explanation from the contract. 
...
This is a deliberate design decision to keep voting and unvoting for stake open to anyone, since it enables the development of off-chain stake pooling protocols. 

\subsection{Kernel Responsibilities}
The only special cases in the kernel are for the Staker Registry contract. Every time a block is produced the kernel must: 
\begin{itemize}
    \item Look-up the staker by signing key (see key indirection section)
    \item Make sure the coinbase claimed in the block header is the coinbase that they registered in the Staker registry (see key indirection section) 
\end{itemize}
Apart from that, the kernel should fix the address of the Staker Registry so it can make special calls to contract state to figure out the total amount of stake committted and stake committed by a particular staker for block rewards, block forging, etc.